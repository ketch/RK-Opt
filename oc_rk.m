function coneq=oc_rk(x,class,s,p)
%function coneq=oc_rk(x,class,s,p)
...
% Order conditions
  ...
  if strcmp(class,'explicitzr')
    A=[zeros(1,s+1);ahat A];
    d=[1;d]; b=[bhat;b]; c=[-1;c];
  end

  tau_2 = (c.^2-d)/factorial(2) - A*c;
  tau_3 = (c.^3+d)/factorial(3) - A*(c.^2)/factorial(2);
  tau_4 = (c.^4-d)/factorial(4) - A*(c.^3)/factorial(3);
  tau_5 = (c.^5+d)/factorial(5) - A*(c.^4)/factorial(4);
  tau_6 = (c.^6-d)/factorial(6) - A*(c.^5)/factorial(5);
  tau_7 = (c.^7+d)/factorial(7) - A*(c.^6)/factorial(6);

  C=diag(c);

  coneq(1)=sum(b)-(1+theta);

  if p<5

      if p>=2
        coneq(2)=b'*c-(1-theta)/2;
      end

      if p>=3
      coneq(3)=b'*c.^2-(1+theta)/3;
      coneq(4)=b'*tau_2;
      end

      if p>=4 
      coneq(5)=b'*c.^3-(1-theta)/4;
      coneq(6)=b'*C*tau_2;
      coneq(7)=b'*A*tau_2;
      coneq(8)=b'*tau_3;
      end
  ...
  elseif p<7 % p>4 and SSP implies stage order >= 2
      coneq(2)=b'*c-(1-theta)/2;
      coneq(3)=b'*c.^2-(1+theta)/3;
      coneq(4)=b'*c.^3-(1-theta)/4;
      coneq(5)=b'*tau_3;
      coneq(6)=b'*c.^4-(1+theta)/5;
      coneq(7)=b'*A*tau_3;
      coneq(8)=b'*tau_4;
      coneq(9)=b'*C*tau_3;

      if p>=6 
        coneq(10)=b'*c.^5-(1-theta)/6;
  ...
  else % assume stage order >= 3
      coneq(2)=b'*c   -(1-theta)/2;
      coneq(3)=b'*c.^2-(1+theta)/3;
      coneq(4)=b'*c.^3-(1-theta)/4;
      coneq(5)=b'*c.^4-(1+theta)/5;
      coneq(6)=b'*tau_4;
      coneq(7)=b'*c.^5-(1-theta)/6;
      coneq(8)=b'*A*tau_4;
      coneq(9)=b'*tau_5;
      coneq(10)=b'*C*tau_4;
      coneq(11)=b'*c.^6-(1+theta)/7;
