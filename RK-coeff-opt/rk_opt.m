...
%     * interp_nodes / interp_values: nodes and target values used to match the stability function.
%     * interp_eval: how the stability function is evaluated inside the interpolation constraint. 'butcher' is the default and evaluates the stability function using the Butcher coefficients. The option 'lowstorage' evaluates it using the 3S* low-storage recursion.
...
[k,np,num_starting_points,startvec,poly_coeff_ind,poly_coeff_val,...
    interp_nodes,interp_values,interp_eval,...
...
i_p.addParameter('interp_nodes',[],@isnumeric);
i_p.addParameter('interp_values',[],@isnumeric);
% Stability-function evaluation used in the interpolation constraints
i_p.addParameter('interp_eval','butcher',@ischar);
...
interp_nodes         = i_p.Results.interp_nodes;
interp_values        = i_p.Results.interp_values;
interp_eval          = i_p.Results.interp_eval;
